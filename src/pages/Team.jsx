...
const Profile = () => {
	...
	useEffect(() => {
		if (teams) {
			const sortedTeams = [...teams].sort((a, b) => b.totalBonus - a.totalBonus);
			const topPerformers = sortedTeams.slice(0, 4);
			const topData = topPerformers.map((top) => ({
				_id: top._id,
				name: top.firstName + " " + top.lastName,
				rank: top.rank,
				totalBonus: top.totalBonus,
			}));
			// console.log(topData);
			setTopPerformer(topData);
			const data = sortedTeams.map((member, idx) => {
				return {
					data: [idx + 1, member.firstName + " " + member.lastName, member.rank, member.totalBonus, member.state],
					_id: member._id,
				};
			});
			setTeam(data);
